...
const headingTextClass = "flex items-center gap-2 text-left text-sm font-semibold text-foreground";
const headingIconClass = "h-4 w-4 text-primary";
const scrollClass = "min-h-0 flex-1";
const listClass = "space-y-4 p-5";
const itemClass = "group flex gap-4 border border-border/60 bg-card p-3 shadow-sm transition-colors hover:border-primary/40 hover:bg-secondary/30";
const imageClass = "h-24 w-20 shrink-0 border border-border object-cover";
const textWrapClass = "min-w-0 flex-1 space-y-1.5";
const brandClass = "text-[11px] font-medium uppercase tracking-[0.22em] text-primary";
const titleClass = "block line-clamp-2 text-sm font-semibold text-foreground transition-colors group-hover:text-primary";
...
const qtyWrapClass = "flex items-center overflow-hidden border border-border bg-background";
const qtyButtonClass = "h-9 w-9 rounded-none px-0";
const qtyValueClass = "flex h-9 min-w-10 items-center justify-center border-x border-border px-2 text-sm font-medium";
const removeButtonClass = "h-9 border-border px-3 text-xs text-white ";
const emptyWrapClass = "flex min-h-[320px] flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-muted/30 px-6 text-center";
const emptyTitleClass = "text-base font-semibold text-foreground";

function CustomerCartItems() {
    const { isSignedIn } = useAuth();

    const { cart, setOpen, increase, decrease, remove } = useCustomerCartAndCheckoutStore((state) => state);
